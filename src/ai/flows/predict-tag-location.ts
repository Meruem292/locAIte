...
const prompt = ai.definePrompt({
  ...
  Provide a brief explanation of your reasoning that can be used as a backtracking plan to find the device. This plan MUST be in a numbered list format (e.g., 1. First, do this. 2. Then, do that.).
  `,
});
...
const predictTagLocationFlow = ai.defineFlow(
  ...
  async input => {
    ...
    if (!output) {
      // Fallback for when the model fails to produce a valid output
      const lastLocation = input.historicalLocations[0];
      return {
        predictedLocation: {
          latitude: lastLocation.latitude,
          longitude: lastLocation.longitude,
          confidence: 0.5,
          reason: '1. The AI model could not determine a likely location. 2. Please check the last known location of the device.',
        }
      };
    }
    return output!;
